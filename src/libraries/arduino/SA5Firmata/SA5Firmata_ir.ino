/*
  SA5Firmata_ir version 5 (is the "ir" version, supporting IR features but with a blocking tone function)
  Pins 2, 5 and 6 are marked as SHIFT mode
  October 2018
  Extending StandardFirmata by Joan Guillén
    ...
    case CAPABILITY_QUERY:
      ...
      for (byte pin = 0; pin < TOTAL_PINS; pin++) {
        if (IS_PIN_DIGITAL(pin)) {
          ...
          Firmata.write(1);
//////////////////////////////////////////////////
//SA5_ir version 5. Marking digital pins 2, 5 and 6
...
//
        //Code adapted from TimerFreeTone lib. Only documented, not used here.
        if (freq <32) {freq = 0;}
...
//
    case 0xCB: //IR receiver
      {
        irrecv.resume();
        unsigned long max_time = millis() + 2000;
        unsigned long irResult = 0;
        while (!irrecv.decode(&results) && millis() < max_time) {
        }
...
// DHT11 sensor  - Changed from 2.1 version
//
  case 0xCF: //DHT11 command
  {
    ...
    while(error == 0 && digitalRead(pin) == LOW)
        if (loopCnt-- == 0) error = 1;
    loopCnt = 10000;
    while(error == 0 && digitalRead(pin) == HIGH)
        if (loopCnt-- == 0) error = 2;
    // READ OUTPUT - 40 BITS => 5 BYTES or TIMEOUT
    for (int i=0; i<40; i++) {
        loopCnt = 10000;
        while(error == 0 && digitalRead(pin) == LOW)
            if (loopCnt-- == 0) error = 3;
        unsigned long t = micros();
        loopCnt = 10000;
        while(error == 0 && digitalRead(pin) == HIGH)
            if (loopCnt-- == 0) error = 4;
        if ((micros() - t) > 30) bits[idx] |= (1 << cnt);
        if (cnt == 0) {  // next byte?
